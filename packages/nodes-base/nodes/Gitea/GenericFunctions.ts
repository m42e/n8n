 ...
 * @returns {Promise<any>}
 */
export async function giteaApiRequest(this: IHookFunctions | IExecuteFunctions, method: string, endpoint: string, body: object, query?: object): Promise<any> { // tslint:disable-line:no-any
	...
	try {
		...
		if (authenticationMethod === 'accessToken') {
			...
			const baseUrl = credentials!.server || 'https://try.gitea.io';
			options.uri = `${baseUrl}${endpoint}`;

			options.headers!.Authorization = `token ${credentials.accessToken}`;
			return await this.helpers.request(options);
		} else {
			const credentials = this.getCredentials('giteaOAuth2Api');

			const baseUrl = credentials!.server || 'https://try.gitea.io';
			options.uri = `${baseUrl}${endpoint}`;
			//@ts-ignore
			return await this.helpers.requestOAuth2.call(this, 'giteaOAuth2Api', options);
		}
	} catch (error) {
		if (error.statusCode === 401) {
			// Return a clear error
			throw new Error('The Gitea credentials are not valid!');
		}

		if (error.response && error.response.body && error.response.body.message) {
			// Try to return the error prettier
			throw new Error(`Gitea error response [${error.statusCode}]: ${error.response.body.message}`);
		}
